...
inline uint64_t CCL_BMRS_X64_is_connected(const uint64_t* flag_bits, unsigned start, unsigned end) {
...
inline void CCL_BMRS_X64_FindRuns(const uint64_t* bits_start, const uint64_t* bits_flag, int height, int width, int qword_width, Run* runs, UFPC& labelsolver) {
	Run* runs_up = runs;
...
void Labeling_BMRS_X64_on_MSB_First(unsigned* dest, const void* source, int height, int width, int data_width, int fmbits) {
	...
	for (int i = 0; i < height / 2; i++) {
		const char* data_u = (const char*)bits + qword_width_in_byte * 2 * i;
