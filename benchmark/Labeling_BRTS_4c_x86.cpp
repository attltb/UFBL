...
void Labeling_BRTS4_X86(unsigned* dest, const void* source, int height, int width, int data_width, int fmbits) {
	...
	private:
		void FindRuns(const uint32_t* bits_start, int width_new, int dword_width, Run* runs, UFPC& labelsolver) {
			...
			const uint32_t* bit_final = bits + width_new / 32 + (width_new % 32 != 0);
...
void Labeling_BRTS4_on_byte_X86(unsigned* dest, const uint8_t* source, int height, int width, int data_width, int fmbits) {
	std::pair<const uint32_t*, int> format_compressed = CCL_Bit_Compressing_X86(source, height, width, data_width);

	Labeling_BRTS4_X86(dest, format_compressed.first, height, width, format_compressed.second, BTCPR_FM_ALIGN_4 | BTCPR_FM_PADDING_ZERO | fmbits);
