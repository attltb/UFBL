...
inline Run* CCL_BRTS4_FindRuns(const unsigned int* bits, const unsigned int* bit_final, Run* runs) {
...
inline void CCL_BRTS4_GenerateLabelsOnRun(Run* runs, Run* runs_end, UFPC& labelsolver) {
	Run* runs_up = runs;
...
void bit_scanning_algorithm(const Data_Compressed& data, Data& data_labels) {
	...
	for (int i = 0; i < height; i++) {
		run_next = CCL_BRTS4_FindRuns(data[i], data[i + 1], run_next);
	}

	UFPC labelsolver;
	labelsolver.Alloc((size_t)((height + 1) / 2) * (size_t)((width + 1) / 2) + 1);
	labelsolver.Setup();
	CCL_BRTS4_GenerateLabelsOnRun(Data_run.runs, run_next, labelsolver);
