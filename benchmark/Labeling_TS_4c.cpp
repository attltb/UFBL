...
void Labeling_TS4(unsigned* dest, const uint8_t* source, int height, int width, int data_width, int fmbits) {
	class TS4 {
		unsigned* dest;
	...
	public:
		TS4(unsigned* _dest, const uint8_t* _source, int _height, int _width, int _data_width, int _fmbits)
	...
	TS4(dest, source, height, width, data_width, fmbits).Perform();
}