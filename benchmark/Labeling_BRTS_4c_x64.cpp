...
inline void CCL_BRTS4_X64_FindRuns(const unsigned long long* bits_start, int height, int width, int qword_width, Run* runs, UFPC& labelsolver) {
	Run* runs_up = runs;
...
void Labeling_BRTS4_X64_on_MSB_First(unsigned* dest, const void* source, int height, int width, int data_width, int fmbits) {
	...
	CCL_BRTS4_X64_FindRuns(bits, height, base_r, qword_width, Data_run.runs, labelsolver);
...
void Labeling_BRTS4_X64(unsigned* dest, const void* source, int height, int width, int data_width, int fmbits) {
	if (fmbits & BTCPR_FM_MSB_FIRST) return Labeling_BRTS4_X64_on_MSB_First(dest, source, height, width, data_width, fmbits);
	...
	labelsolver.Setup();
	Runs Data_run(height, width);
	CCL_BRTS4_X64_FindRuns(bits, height, width, qword_width, Data_run.runs, labelsolver);
