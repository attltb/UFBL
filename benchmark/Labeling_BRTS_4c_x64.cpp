...
void Labeling_BRTS4_X64(unsigned* dest, const void* source, int height, int width, int data_width, int fmbits) {
	...
	public:
		BRTS4(unsigned* _dest, const void* _source, int _height, int _width, int _data_width, int _fmbits)
		...
		void Perform()
		{
			...
			FindRuns(bits, width, qword_width, Data_run.runs, labelsolver);
		...
		void Perform_on_MSB_First()
		{
			...
			labelsolver.Setup();
			Runs Data_run(height, width);
			FindRuns(bits, width_new, qword_width, Data_run.runs, labelsolver);
	...
	private:
		void FindRuns(const uint64_t* bits_start, int width_new, int qword_width, Run* runs, UFPC& labelsolver) {
			Run* runs_up = runs;

			//process runs in the first row
			const uint64_t* bits = bits_start;
			const uint64_t* bit_final = bits + width_new / 64 + (width_new % 64 != 0);
